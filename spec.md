...
TXT リソースレコードにおけるフォーマットは下記の通り、項目名と項目値の組を ASCII 等号('=')文字で連結した項目を、ASCII 空白(' ')文字区切りで記述したものとし(両端の二重引用符は含まない)、単一の TXT リソースレコードのみを返すこととする。

    "v=v6mig-1 url=https://vne.example.jp/rule.cgi t=b"

各項目は以下の順にすべて記述しなければならない(MUST)。
...
- `order`
  - マイグレーション方式を優先度順に並べたもの。プロビジョニング情報に複数の方式が記述されている場合、CPE は order に記述された順序で各方式の利用を試みる。
...
- c) プロビジョニングサーバのアドレスと通信方式が取得できたか
  - c.1) 取得した TXT リソースレコードが3.1節.の規定を満たす場合
    - d) へ
  - c.2) 取得した TXT リソースレコードが3.1節.の規定を満たさない、または NODATA もしくは NXDOMAIN で取得できない場合
...
- f) プロビジョニングサーバから応答があったか
  - f.1) HTTP ステータスコードが200の場合
    - g) へ
  - f.2) HTTP ステータスコードが307の場合
    - Locationで示されたプロビジョニングサーバ URL を用いて、d)へ
...
- g) プロビジョニング情報を解析する
  ...
  - g.2) プロビジョニング情報が3.4節の規定を満たさない場合
    - 10-30 分間のランダムな時間を待つ
    - a) へ
- h) 以前にマイグレ技術が設定されて有効か
  - h.1) 有効な場合
    - 以前の設定から変更があれば i) へ
    - 以前の設定から変更がなければ j) へ
  - h.2) 無効な場合
    - i) へ
- i) プロビジョニング情報に合わせて、CPE を設定する
...
プロビジョニングサーバは、CPE の IPv6 アドレスに応じてプロビジョニング情報の内容を変化させる場合がある。そのため、CPE は自身の IPv6 アドレスの変化を検出した際には、プロビジョニング情報を再取得すること(SHOULD)。
